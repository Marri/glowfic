...
class IconsController < UploadingController
  ...
  def delete_multiple
    deleter = Icon::MultiRemover.new
    deleter.perform(params, user: current_user)
    if deleter.errors.present?
      flash[:error] = deleter.errors.full_messages.first
      redirect_to user_galleries_path(current_user)
    else
      flash[:success] = "Icons #{params[:gallery_delete] ? 'removed from gallery' : 'deleted'}."
